...
def dqn(n_episodes=4000, max_t=1000, eps_start=1.0, eps_end=0.01, eps_decay=0.995):
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        wandb.log({'Episode': i_episode, 'Reward': score, 'Epsilon': eps, 'Num Time Steps': t, 'Average Reward': np.mean(scores_window)})

        if np.mean(scores_window)>=300.0:
            print('\nEnvironment solved in {:d} episodes!\tAverage Score: {:.2f}'.format(i_episode-100, np.mean(scores_window)))
            torch.save(agent.qnetwork_local.state_dict(), 'saved_models/baseline-train.pth')
            break
    return scores
